...
def _plan_tools(llm: LLM, command: dict) -> list[dict]:
    ...
    # 硬过滤：移除 whatweb（Ruby依赖已坏）
    tools = [t for t in tools if t.get("tool") != "whatweb"]
    if not tools:
        print("[情报Agent] 所有工具被过滤，回退到仅 nmap")
        tools = [{"tool": "nmap", "args": ["-sT", "-Pn", "-sV", "--top-ports", "100", command.get("target", "localhost")]}]
...
你会收到安全工具的原始输出（nmap、subfinder 等）。
